...
let TokenService = class TokenService {
    ...
    async update(userId, token, tokenType) {
        try {
            if (tokenType === 'refresh') {
                return await this.tokenModel
                    .findOneAndUpdate({ userId: userId }, { refreshToken: token }, { new: true })
                    .exec();
            }
            else if (tokenType === 'reset') {
                return await this.tokenModel
                    .findOneAndUpdate({ userId: userId }, { resetPasswordToken: token }, { new: true })
                    .exec();
            }
        }
        catch (error) {
