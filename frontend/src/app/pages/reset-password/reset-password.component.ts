  ...
  private checkErrorStatusCodeAndRedirect(err): void {
    if (err.error.statusCode === 403) {
      this.notificationService.error(
        'Tokenul a expirat. Va rugam sa solicitati un nou link de resetare a parolei'
      );
    } else {
      this.notificationService.error('Nu a fost oferit un token valid');
    }
    this.storageService.removeItem('tokens');
    this.loginDataService.setNextLoggedUser(null);
    this.router.navigate(['/ai-uitat-parola']);
  }
